...
export class Cgtrans {
    ...
    normalizeRupiah1(value) { // Untuk format pemisah ribuan dengan koma
        if (!value) return 0;
    ...
    normalizeRupiah2(value) { // Untuk format pemisah ribuan dengan titik
        if (!value) return 0;
    
        return Number(
            value
                .replace(/Rp\s*/i, '') //hapus Rp
                .replace(/\./g, '')    //hapus pemisah ribuan
                .replace(',', '.')     //ubah desimal Indonesia (koma) ke format JS (titik)
                .trim()
        );
    }
    ...
    async validasiHargaTiketKursi(harga_tiket, jml_penumpang, kursi_tersedia) { //Validasi harga tiket yang terpampang di kursi
        ...
        if (harga_type === "range") {
            ...
            harga_min = this.normalizeRupiah1(harga_min);
            ...
            for (let i = 0; i < jml_penumpang; i++) {
                ...
                if (await kursi_tersedia.nth(i).locator('p').filter({ hasText : /Sale|Promo/i }).count() > 0) {
                    harga_kursi = Math.round(this.normalizeRupiah2(await kursi_tersedia.nth(i).locator('span').nth(2).innerText()));
                } else {
                    harga_kursi = Math.round(this.normalizeRupiah2(await kursi_tersedia.nth(i).locator('span').nth(1).innerText()));
                }
    ...
    async validasiTotalHargaTiket(harga_tiket, jml_penumpang, expected_total_tiket, current_page, biaya_lainnya, case_flag) {
        ...
        switch(current_page) {
                ...
                if (await this.validasiHargaTiketKursi(harga_tiket, jml_penumpang, list_kursi_tersedia)) {
                    ...
                    for (let i = 0; i < jml_penumpang; i++) {
                        ...
                        if (await list_kursi_tersedia.nth(i).locator('p').filter({ hasText : /Sale|Promo/i }).count() > 0) {
                            current_harga_tiket = Math.round(this.normalizeRupiah2(await list_kursi_tersedia.nth(i).locator('span').nth(2).innerText()));
                        } else {
                            current_harga_tiket = Math.round(this.normalizeRupiah2(await list_kursi_tersedia.nth(i).locator('span').nth(1).innerText()));
                        }
            ...
            case("payment-page") :
                const actual_total_tiket_payment_1 = this.normalizeRupiah1(await this.page.locator('div:has-text("Total Bayar") + div > p ').innerText());
                const actual_total_tiket_payment_2 = this.normalizeRupiah1(await this.page.locator('span#hargatot').innerText());
