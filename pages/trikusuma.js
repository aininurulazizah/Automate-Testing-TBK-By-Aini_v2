...
export class Trikusuma {
    constructor(page){
        ...
        this.antar_jemput_field = page.locator('select#jenis_antar_jemput + div');
        this.alamat_jemput_pemesan = page.locator('#alamat_jemput');
        this.alamat_antar_pemesan = page.locator('#alamat_antar');
        this.area_jemput_pemesan = page.locator('#area_jemput + div');
        this.area_antar_pemesan = page.locator('#area_antar + div');
    ...
    async isiDataPenumpang(jml_penumpang, pemesan, penumpang, biaya_lainnya) {
        await this.waitForLoader('div#modal-load', 'show', false);
        ...
        await this.antar_jemput_field.click();
        await this.antar_jemput_field.locator(`div.ss-option:has-text("${pemesan.JenisAntarJemput}")`).click();

        if (pemesan.JenisAntarJemput === "Jemput - Antar" || pemesan.JenisAntarJemput === "Hanya Jemput") {
            await this.alamat_jemput_pemesan.fill(pemesan.AlamatJemput);
            await this.area_jemput_pemesan.click();

            const area_jemput = this.area_jemput_pemesan.locator('div.ss-option').nth(1);
            await area_jemput.click();
            const harga_jemput = this.normalizeRupiah((await area_jemput.textContent()).split(' - ')[1].trim());
            biaya_lainnya.Tambahan[0].biaya_antar += harga_jemput;
        }
        
        if (pemesan.JenisAntarJemput === "Jemput - Antar" || pemesan.JenisAntarJemput === "Hanya Antar") {
            await this.alamat_antar_pemesan.fill(pemesan.AlamatAntar);
            await this.area_antar_pemesan.click();

            const area_antar = this.area_antar_pemesan.locator('div.ss-option').nth(3);
            await area_antar.click();
            const harga_antar = this.normalizeRupiah((await area_antar.textContent()).split(' - ')[1].trim());
            biaya_lainnya.Tambahan[0].biaya_jemput += harga_antar;
    ...
    async validasiTotalHargaTiket(harga_tiket, jml_penumpang, expected_total_tiket, current_page, biaya_lainnya, case_flag) {
        ...
        switch(current_page) {
            ...
            case("payment-page") :
                ...
                for (const list of biaya_lainnya.Tambahan) {
                    for (const [biaya, nominal] of Object.entries(list)) {
