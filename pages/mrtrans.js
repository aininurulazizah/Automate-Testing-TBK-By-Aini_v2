...
export class Mrtrans {
    ...
    async validasiHargaTiketKursi(harga_tiket, jml_penumpang, kursi_tersedia) { //Validasi harga tiket yang terpampang di kursi
        ...
        if (harga_type === "range") {
            ...
            for (let i = 0; i < jml_penumpang; i++) {
                const kursi = await kursi_tersedia.nth(i);
                const onclick = await kursi.getAttribute('onclick');
                const args = onclick.match(/'([^']*)'/g).map(v => v.replace(/'/g, ''));
                const [id, harga, promo] = args;
                const harga_kursi = this.normalizeRupiah(harga);
    ...
    async validasiTotalHargaTiket(harga_tiket, jml_penumpang, expected_total_tiket, current_page, biaya_lainnya) {
        ...
        switch(current_page) {
            ...
            const list_kursi_tersedia = await this.kursi_tersedia;
            let expected_temp = 0;

                if (await this.validasiHargaTiketKursi(harga_tiket, jml_penumpang, list_kursi_tersedia)) {

                    for (let i = 0; i < jml_penumpang; i++) {
                        const kursi = await list_kursi_tersedia.nth(i);
                        const onclick = await kursi.getAttribute('onclick');
                        const args = onclick.match(/'([^']*)'/g).map(v => v.replace(/'/g, ''));
                        const [id, harga, promo] = args;
                        const current_harga_tiket = this.normalizeRupiah(harga);
